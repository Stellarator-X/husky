...
const float MAX_DIST = 3.5; // Threshold Distance for the algo
const float WIDTH = 0.5;//Width of the robot
...
float mapx(float current, float goal, float max){
    return max*abs(current-goal);
}
